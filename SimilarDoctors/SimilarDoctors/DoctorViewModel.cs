...
namespace SimilarDoctors
{
    public class DoctorViewModel
    {
        ...
        public static List<DoctorViewModel> ListOfDoctors = ListOfDoctorsViewModel.GetInstance().ListOfDoctors;
        ...
        /// And filter the dictionary with respect ti input threshold similarity weight.
        /// </summary>
        /// <param name="pThresholdSimilarityWeight">indicates the minimum threshold similarity weight.</param>
        public void SortAndFilterSimilarityWeights(int pThresholdSimilarityWeight)
        {
            // Remove entries with SW value < Threshold SW value
            this.SimilarityWeights = this.SimilarityWeights.Where(x => x.Value > pThresholdSimilarityWeight).ToDictionary(x => x.Key, x => x.Value);

            // Sort dictionary SW in descending order with respect to assinged SW value. 
            this.SimilarityWeights = this.SimilarityWeights.OrderByDescending(x => x.Value).ToDictionary(x => x.Key, x => x.Value);
        }
